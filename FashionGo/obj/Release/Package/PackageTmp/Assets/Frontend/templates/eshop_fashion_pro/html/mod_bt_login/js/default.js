...
function loginAjax() {
	...
	if(BTLJ("#btl-input-username").val()=="") {
		showLoginError("Bạn chưa nhập tên đăng nhập!");
		return false;
	}
	if(BTLJ("#btl-input-password").val()==""){
		showLoginError('Bạn chưa nhập mật khẩu.!');
		return false;
	}
	var token = BTLJ('.btl-buttonsubmit input:last').attr("name");
	var value_token = encodeURIComponent(BTLJ('.btl-buttonsubmit input:last').val()); 
	var datasubmit= "bttask=login&Email="+encodeURIComponent(BTLJ("#btl-input-username").val())
	+"&Password=" + encodeURIComponent(BTLJ("#btl-input-password").val())
	+ "&"+token+"="+value_token
	+"&return="+ encodeURIComponent(BTLJ("#btl-return").val());
	
	if(BTLJ("#btl-checkbox-remember").is(":checked")){
	    datasubmit += '&RememberMe=1';
	}
